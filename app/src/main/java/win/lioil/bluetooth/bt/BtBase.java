                ...
                switch (in.readInt()) {
                    ...
                    case FLAG_FILE: //读取文件
                        ...
                        while ((r = in.read(b)) != -1) {
                            out.write(b, 0, r);
                            len += r;
                            float progress = (float) len/(float)fileLen;
                            BigDecimal d= new BigDecimal(progress);
                            float progress1 = d.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
                            notifyUI(Listener.MSG, "正在接收文件(" + fileName + "),请稍后..."+progress1*100+"%");
                            if (len >= fileLen)
                                break;
                        }
                        Log.e("Accept-FILE:","文件接收完成");
        ...
        Util.EXECUTOR.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    long sendFileLen = file.length();
                    Log.e("Send-FILE:","文件发送开始");
                    int r;
                    byte[] b = new byte[8 * 1024];
                    while ((r = in.read(b)) != -1){
                        mOut.write(b, 0, r);
                        sendFileLen -=r;
                        float progress = 1-(float)sendFileLen/(float)fileLen;
                        BigDecimal d= new BigDecimal(progress);
                        float progress1 = d.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
                        notifyUI(Listener.MSG, "正在发送文件(" + filePath + "),请稍后..."+progress1*100+"%");
                    }

                    mOut.flush();
                    Log.e("Send-FILE:","文件发送完成");
